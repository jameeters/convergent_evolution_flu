...
mutations_ha = mutations %>% filter(gene == 'HA')
...
ggplot(dn_ds, aes(x=pos_aa, y=ratio)) +
  ...
  facet_wrap(vars(host, gene), ncol=1, labeller = label_wrap_gen(multi_line=FALSE)) +
...
#### PB2
```{r plot_dms_comparison_pb2, include=T, echo=F, warning=F, fig.height=11, fig.width=11}
...
```{r plot_dms_comparison_pb2_error_bars, include=T, echo=F, warning=F, fig.height=11, fig.width=11}
ggplot(dn_ds_dms_pb2, aes(x=wt_mean_dms_value, y=ratio, color=pos_aa)) +
  geom_point() +
  scale_color_continuous(type='viridis') + 
  geom_errorbarh(aes(xmin=min_dms_value, xmax=max_dms_value), height=0.06) +
  ggtitle('PB2, B3.13') +
  ylab('dN/dS') +
  facet_wrap(vars(dms_name, host), scales='free_x', ncol=4) +
  geom_text_repel(aes(label=pos_aa), color='black', max.overlaps=5, size=4)
